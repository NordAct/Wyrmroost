...
{
    public static final ResourceLocation BLUE_FIRE = Wyrmroost.rl("textures/entity/projectiles/rr_breath/blue_fire.png");
    ...
    private void renderFire(PoseStack ms, MultiBufferSource typeBuffer, Entity entity)
    {
        ...
        float z = 0;
        VertexConsumer vertex = typeBuffer.getBuffer(RenderType.entityCutoutNoCull(BLUE_FIRE));
        PoseStack.Pose msEntry = ms.last();

        int firstFrame = 16;
        float v1 = 1f/32f * ((entity.tickCount + firstFrame) % 32);
        float v0 = 1f/32f * (((entity.tickCount + firstFrame) % 32) + 1);

        vertex(msEntry, vertex, x, -y, z, 1, v0);
        vertex(msEntry, vertex, -x, -y, z, 0, v0);
        vertex(msEntry, vertex, -x, 1.4f - y, z, 0, v1);
        vertex(msEntry, vertex, x, 1.4f - y, z, 1, v1);

        ms.popPose();
    }

    private static void vertex(PoseStack.Pose msEntry, VertexConsumer bufferIn, float x, float y, float z, float texU, float texV) {
        bufferIn.addVertex(msEntry.pose(), x, y, z).setColor(0xFFFFFFFF).setUv(texU, texV)
                .setOverlay(OverlayTexture.NO_OVERLAY)
                .setLight(LightTexture.FULL_BRIGHT)
                .setNormal(msEntry, 0.0F, 1.0F, 0.0F);
    }
}
